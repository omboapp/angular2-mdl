...
export class EntryPointDoc {
  ...
  /** Name of the package, either mdl or cdk */
  packageName: string;
...
export class EntryPointGrouper implements Processor {
  ...
  $process(docs: DocCollection) {
    ...
    docs.forEach(doc => {
      ...
      const packageDisplayName = packageName === 'cdk' ? 'CDK' : 'Material Design Lite';
...
function getModulePackageInfo(doc: Document): ModuleInfo {
  ...
  // All of the component documentation is under either `src/mdl` or `src/cdk`.
  ...
  // guaranteed because we can also export a module from `mdl/common`. e.g. the ripple module.
  let moduleName = pathSegments[1];

  // The ripples are technically part of the `@angular/mdl/common` entry-point, but we
  // want to show the ripple API separately in the docs. In order to archive this, we treat
  // the ripple folder as its own module.
  if (pathSegments[1] === 'common' && pathSegments[2] === 'ripple') {
    moduleName = 'ripple';
  }
