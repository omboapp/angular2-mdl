...
//																													console.warn('bundlePrimaryEntryPoint bundleEntryPoint.importName', `@angular/mdl/${packageName}`);
...
//																													console.warn('bundleSecondaryEntryPoint bundleEntryPoint.importName', `@angular/mdl/${packageName}/${entryPointName}`);
  ...
  private async _createRollupBundle(config: RollupBundleConfig) {
    ...
    if (config.format === 'umd') {
      ...
      if (this._buildPackage.exportsSecondaryEntryPointsAtRoot &&
        ...
        // Use the rollup-alias plugin to map imports of the form `@angular/mdl/button`
...
//																													console.warn('createRollupBundle writeOptions.name', writeOptions.name);

    return rollup.rollup(bundleOptions).then((bundle: any) => bundle.write(writeOptions));
  }

  /**
   * Gets mapping of import aliases (e.g. `@angular/mdl/button`) to the path of the es5
   * bundle output.
  ...
  private _getAmdModuleName(packageName: string, entryPointName?: string) {
    // For example, the AMD module name for the "@angular/mdl-examples" package should be
    // "ng.mdlExamples". We camel-case the package name in case it contains dashes.
    let amdModuleName = `ng.${dashCaseToCamelCase(packageName)}`;

    if (entryPointName) {
      // For example, the "@angular/mdl/icon-toggle" entry-point should be converted into
      // the following AMD module name: "ng.mdl.iconToggle". Similar to the package name,
