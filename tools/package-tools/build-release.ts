...
function copyPartialToRootAndUpdateImports(
  ...
  for (let packageName of packageDirs) {
    ...
    if (packageName === 'mdl') {
      // If importing from the `mdl` package, the rest of the path can be anything (it doesn't
      // necessarily have to be a secondary entry point, since `@angular/mdl` bundles all of its
      // sass into `@angular/mdl/theming` before releasing. We just need to make sure the rest
    ...
    if (packageName === 'mdl') {
      // If we're importing from `@angular/mdl`, update the first import to the combined bundle
      // and just remove the rest. (This is because `@angular/mdl` combines all of its Sass
      ...
      sassPartialData = sassPartialData.replace(importRegex, (match) => {
        const result = updated ? '' : `@import '../mdl/theming';`;
        updated = true;
